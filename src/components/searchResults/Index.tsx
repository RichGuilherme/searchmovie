...
export const SearchResults = ({ queryProps }: searchResultsProps) => {
    ...
    const { data: _data, loading } = useFetch(`https://api.themoviedb.org/3/search/multi?query=${queryProps}&page=${pageCurrent}`)
    ...
    return (
        <section className="w-full font-OpenSans text-textColors-200">
            ...
            <div className="w-[83%] mx-auto mt-36  relative  backdrop-blur-[3px] bg-[rgba(0,_0,_0,_.25)]">
                ...
                <div className="grid grid-cols-[repeat(auto-fill,267px)] justify-center gap-x-5 gap-y-10 mt-[24px] p-4">
                    <CardItem data={data} loading={loading} />
                </div>


                <nav className="mx-auto flex flex-row items-center gap-5 justify-center my-12">
                    {pageCurrent > 1 &&
                        <SlArrowLeft
                            onClick={() => handlerPage(pageCurrent - 1)}
                            size={25}
                        ...
                        {numberPage && Array.from({ length: numberPage }).map((_, index) => {
                            ...
                            return (
                                <div
                                    ...
                                    key={index}
                                    className="flex items-center justify-center w-8 h-8 rounded-full border-[2px] border-textColors-100 font-medium
